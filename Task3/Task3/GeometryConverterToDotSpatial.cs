...
namespace GeoAPI.Geometries
{
    ...
    public static class GeometryConverter
    {
        ...
        private static DSShapeRange ShapeRangeFromGeometry(IGeometry geometry, double[] vertices, int offset)
        {
            ...
            var shx = new DSShapeRange((DotSpatial.Data.FeatureType)featureType) { Extent = geometry.EnvelopeInternal.ToDotSpatial(), };
            var vIndex = offset / 2;
#if DS16
            ...
            {
                var prtx = new DSPartRange(vertices, shapeStart, vIndex - shapeStart, (DotSpatial.Data.FeatureType)featureType);
                ...
                if (bp != null)
                {
                    ...
                    {
                        var holex = new DSPartRange(vertices, shapeStart, vIndex - shapeStart, (DotSpatial.Data.FeatureType)featureType)
                        {
        ...
        {
            Console.WriteLine(NetTopologySuite.Algorithm.Orientation.IsCCW((NetTopologySuite.Geometries.CoordinateSequence)ring.CoordinateSequence) == ccw);

            if (NetTopologySuite.Algorithm.Orientation.IsCCW((NetTopologySuite.Geometries.CoordinateSequence)ring.CoordinateSequence) == ccw)
                return ring;
