...
{
    /// <summary>
    /// This MapPolygonLayer is able to load itself on deserialization even if the contained DataSet doesn't know how to load the file.
    /// </summary>
    public class MapSelfLoadPolygonLayer : MapPolygonLayer, IMapSelfLoadLayer
    {
        #region Fields
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="MapSelfLoadPolygonLayer"/> class.
        /// </summary>
        /// <param name="featureSet">The FeatureSet that contains the layers data.</param>
        public MapSelfLoadPolygonLayer(IFeatureSet featureSet)
            : base(featureSet)
        {
        ...
        public static MapSelfLoadPolygonLayer Open(string filePath)
        {
            var dataset = DataManager.DefaultDataManager.OpenFile(filePath) as ISelfLoadSet;
            var layer = dataset?.GetLayer() as MapSelfLoadPolygonLayer;
            return layer;
        }
